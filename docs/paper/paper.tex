% Posted on arXiv under the arXiv.org perpetual, non-exclusive license.
% The author retains all copyright; arXiv is granted only the right to
% distribute this version. https://arxiv.org/licenses/nonexclusive-distrib/1.0/
...
\IEEEauthorblockA{pollinations.ai\\ayushman@pollinations.ai}
\and
\IEEEauthorblockN{Nihal Gazi}
\IEEEauthorblockA{pollinations.ai\\info@nihalgazi.com}
}
...
\subsection{Cost Comparison: Commercial AI Search vs.\ OreoLook}
...
\begin{table}[h]
\centering
\caption{Measurement-period cost comparison vs.\ OreoLook}
\label{tab:cost}
{\scriptsize
\begin{tabular}{@{}lrrl@{}}
\toprule
...
\midrule
OreoLook (uncached) & \$0.015 & \$1{,}596 & infra + LLM \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{axis}[
    ...
    symbolic x coords={{SearchGPT}, {Google Gemini}, {Perplexity}, {OreoLook}},
    xtick=data,
...
({OreoLook}, 0.015) [\$0.015]
};
\end{axis}
\end{tikzpicture}%
}
\caption{Measurement-period per-query estimates (log scale). OreoLook includes amortized local infrastructure and provider inference; no cached-cost estimate is inferred from the Redis keyspace hit rate.}
\label{fig:cost}
\end{figure}
