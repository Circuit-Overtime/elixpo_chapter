...
async function collectMetrics(db, userId) {
  ...
  const [
    ...
    db.prepare(`
      ...
      WHERE b.author_id = ? AND b.status = 'published' AND b.secret = 0 AND b.read_time_minutes >= 7
        AND (SELECT COALESCE(AVG(CASE WHEN ae.event_type = 'read_complete' THEN 1 ELSE ae.event_value END), 0)
             FROM analytics_events ae WHERE ae.blog_id = b.id AND ae.event_type IN ('read_progress', 'read_complete')) >= .4
    `).bind(userId).first(),
    ...
    db.prepare("SELECT DISTINCT CAST(strftime('%Y', published_at, 'unixepoch') AS INTEGER) * 53 + CAST(strftime('%W', published_at, 'unixepoch') AS INTEGER) week_key FROM blogs WHERE author_id = ? AND status = 'published' AND secret = 0 ORDER BY week_key").bind(userId).all(),
    db.prepare(`
    ...
    `).bind(userId, userId).first(),
    db.prepare(`
      SELECT COUNT(DISTINCT b.id) stories, COUNT(DISTINCT peer_id) peers FROM (
        SELECT b.id, ca.user_id peer_id FROM blogs b JOIN blog_co_authors ca ON ca.blog_id = b.id
        WHERE b.author_id = ? AND b.status = 'published' AND ca.status = 'accepted'
        UNION ALL
        SELECT b.id, b.author_id peer_id FROM blogs b JOIN blog_co_authors ca ON ca.blog_id = b.id
        WHERE ca.user_id = ? AND ca.status = 'accepted' AND b.status = 'published'
      ) x JOIN blogs b ON b.id = x.id
    `).bind(userId, userId).first(),
    ...
    db.prepare(`
      SELECT COUNT(*) c FROM (
        ...
        JOIN blogs b ON b.published_as = 'org:' || o.id AND b.status = 'published'
